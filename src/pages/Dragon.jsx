      ...
      {dragon.map((dra, i) => (
        <div key={i} className='flex px-20 my-10 mr-60 mx-5'>
          ...
          <div className='ml-5'>
            ...
            <Button className= {dra.reservation === false ? "p-2 bg-primary rounded-lg opacity-50 text-font " :'text-secondary rounded-lg border-solid border-2 border-grey-600 p-2'} event = {() => handeleReserve(dra.id)} buttonTag={dra.reservation === false ?'Reserve Dragon' : 'cancelReservation'} />
          </div>
        </div>
      ))}
